...
namespace ProjectColoni
{
	public class GoapAgent : MonoBehaviour {
		...
		public FSM.FSMState idleState; // finds something to do
		...
		private HashSet<KeyValuePair<string, object>> _worldState;
		private HashSet<KeyValuePair<string, object>> _goal;
		...
		private void createIdleState() {
			idleState = (fsm, aiStateController) => {
				...
				if (plan != null) {
					...
					currentActions = plan;
					dataProvider.planFound(_goal, plan);
		...
		private void createPerformActionState() {
			...
			performActionState = (fsm, stateController) => {
				// perform the action
				...
				if (hasActionPlan()) {
					...
					if ( inRange ) {
						...
						bool success = action.perform(stateController);
